...
import 'package:game_tracker/pages/edit_game.dart';
import 'package:game_tracker/pages/review_page.dart';

class GameDetailGuest extends StatefulWidget {
  final VoidCallback signOut;
  final int userId;
  final Game game;

  GameDetailGuest({required this.signOut, required this.userId, required this.game});

  @override
  State<GameDetailGuest> createState() => _GameDetailGuestState();
}

class _GameDetailGuestState extends State<GameDetailGuest> {
  ...
  Widget _editBtn() {
    return ElevatedButton(
      onPressed: () {
        Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => EditGamePage(signOut: signOut, game: widget.game, userId: widget.userId)),
          );
      },
      style: ElevatedButton.styleFrom(
        backgroundColor: Color.fromARGB(255, 109, 49, 237),
